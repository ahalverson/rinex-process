...
def analyze_volatility(df: pd.DataFrame, start_date: str, end_date: str, period: str, period_name: str):
    """
    Analyzes the volatility of ephemeris parameters using Plotly for visualization.
    """
    print("\n" + "="*80)
    ...
    if len(period_deltas) < 10:
        ...
        # Create a subplot figure with titles for each plot
        fig = make_subplots(
            rows=len(params_to_plot), cols=1,
            subplot_titles=[f'Distribution of {period_name} Change in {p}' for p in params_to_plot]
        )

        # Loop through parameters and add a histogram trace for each
        for i, param in enumerate(params_to_plot):
            if param in period_deltas.columns:
                # Add the histogram to the correct subplot
                fig.add_trace(
                    go.Histogram(
                        x=period_deltas[param].dropna(),
                        name=param,
        ...
        fig.update_layout(
            title_text=f'Interactive Distribution of {period_name} Ephemeris Changes ({start_str} to {end_str})',
            height=400 * len(params_to_plot),
            showlegend=False,
        ...
        # Save the figure to an interactive HTML file
...
def get_date_from_user(prompt_text: str) -> str:
    ...
    while True:
        ...
        try:
            pd.to_datetime(date_str); return date_str
        except ValueError:
